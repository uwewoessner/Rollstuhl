...
void sendDeviceInfo(IPAddress destinationAddress)
{
  ...
  strcpy(buffer, "devInfo Wheelchair ");
...
void setup()
{
  ...
  else
  {
    ...
    ArduinoOTA.setHostname("Rollstuhl");
  ...
  configButton.init(true);
	leftEncoder.attachHalfQuad(22, 23);
	rightEncoder.attachHalfQuad(26, 27);
...
void loop()
{
  ...
  {
    mb.state = 0;
    mb.countLeft = leftEncoder.getCount();
    mb.countRight = rightEncoder.getCount();
  ...
  {
    Serial.println("Encoder count = " + String((int32_t)leftEncoder.getCount()) + " " + String((int32_t)rightEncoder.getCount()));
  }
  delay(100);
